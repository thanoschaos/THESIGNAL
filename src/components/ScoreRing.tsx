    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="font-tech text-[10px] tracking-[0.3em] mt-1 text-white/50 uppercase">
            {getScoreLabel(score)}
          </span>
        </div>
      </div>
      {change !== undefined && (
        <div className="flex items-center gap-1.5">
          <span className={`font-tech text-[10px] tracking-[0.2em] uppercase ${change >= 0 ? "text-[#adff00]" : "text-[#ff00e5]"}`}>
            {change >= 0 ? "▲" : "▼"} {Math.abs(change)} PTS
          </span>
          <span className="font-tech text-[10px] tracking-[0.2em] text-white/40 uppercase">VS LAST</span>
        </div>
      )}
    </div>
  );
